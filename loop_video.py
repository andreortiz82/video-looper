    ...
    def make_frame(t):
        ...
        if len(chunk) > 1:
            ...
            if len(fft_slice) >= N_BARS:
                ...
                bar_vals = np.array([np.mean(fft_slice[edges[i]:edges[i + 1] + 1]) for i in range(N_BARS)])
...
def render_one(song_path, song_name, video_clip, canvas_color, youtube, viz_choices):
    ...
    layers = [ColorClip(size=canvas_size, color=canvas_color, duration=audio_clip.duration)]
    ...
    if 3 in viz_choices:
        ...
        layers.append(make_pulsing_border_clip(mono, global_max_rms, audio_clip.duration, canvas_size, cx, cy, video_clip.w, video_clip.h, viz_color, canvas_color))

    layers.append(looped_video.with_position((cx, cy)))

    if 1 in viz_choices:
        layers.append(make_waveform_clip(mono, audio_clip.duration, VIZ_WIDTH, WAVEFORM_HEIGHT, viz_color, canvas_color).with_position((viz_x, viz_y)))
        viz_y += WAVEFORM_HEIGHT + 12

    if 2 in viz_choices:
        layers.append(make_freq_bars_clip(mono, audio_clip.duration, VIZ_WIDTH, FREQ_BAR_HEIGHT, viz_color, canvas_color).with_position((viz_x, viz_y)))
...
for dest in sorted(destinations):
    ...
    label = "YT" if youtube else "INSTA"
    print(f"\n--- Rendering {label} ---")
    output = render_one(song_path, song_name, video_clip, canvas_color, youtube, viz_choices)
